...
// Globales
const PUERTO:number|string = process.env.PORT || 8080;
const app = express();
const http = require("http").Server(app);
const io = require("socket.io")(http);
const mensajes:{time_info:string,mail:string,mensaje:string}[] = []; 
type data = {
    time_info:string,
    mail:string,
    mensaje:string
...
// Funciones 
const miGuardado = (data:data):void =>{
    // Guarda historial
...
function leerHistorial(socket:any){
    fs.readFile(__dirname + "/public/mensajes.txt",(error:Error,data:string) =>{
        if(error){
            console.log(error);
        }else{
...
io.on("connection",(socket:any)=>{
    ...
    socket.on("productos-payload",(data:payload)=>{
        console.log(data);
        io.sockets.emit("productos",data);
    });
    socket.on("mensaje",(data:data)=>{
